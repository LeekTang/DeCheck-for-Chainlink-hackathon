    ...
    <div class="bg-[#070312] min-h-screen px-[11px] pb-[60px]">
        ...
        <div class="pt-[16px] min-h-screen overflow-y-auto overscroll-y-contain" style="font-family: Hezaedrus-Medium;">
            <div class="flex flex-wrap  w-[100%]">
                <div class="p-[5px] w-[50%]" v-for="({ content, pic, btnText, callback ,disable}, index) in list" :key="index">
                    <card :content="content" :pic="pic" :btnText="btnText" :submit="() => !disable && callback(index)"></card>
                </div>
            </div>
        ...
        <van-popup v-model:show="popup"  class="bg-[#1B1A1D]" position="bottom" :style="{ height: '85%' }">
          <div class="bg-[#1B1A1D] h-[100%] p-[24px] text-center" style="font-family: Hezaedrus-Medium;">
                ...
                <div class="h-[32px] text-[16px] bg-[white] rounded-[12px] leading-[32px]" @click="submit" round>
...
const activePopup = (index) => {
    state.popup = true;
    console.log('list[index]',state.list[index]);
